...
function heroFrames() {
  ...
  const seen = new Set();
  products.forEach((product) => (product.colors || []).forEach((color) => {
    [color.files?.[0], color.video].forEach((file) => {
...
function homePage() {
  ...
  const selected = products.filter((p) => ['aura','cream-icon','high-icon','fitted-wool'].includes(p.id));
