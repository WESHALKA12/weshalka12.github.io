...
function catalogPage(collectionOnly = false) {
  ...
  const params = new URLSearchParams(location.search); if (params.get('filter') === 'tall') $('[data-filter="tall"]').value = 'yes'; if (GROUPS.some((group) => group.id === params.get('group'))) $('[data-filter="group"]').value = params.get('group'); if (params.get('type') === 'coats') { document.querySelectorAll('[data-card]').forEach((card) => { if (card.dataset.group.includes('fur')) card.dataset.typeHidden = 'yes'; }); } if (params.get('type') === 'fur') { $('[data-filter="group"]').value = 'fur'; }
