...
const asset = (file = '') => /^(\/|https?:)/.test(file) ? file : `/assets/images/${file}`;
...
let products = [];
let site = {contacts:{}, home:{}, sale:{items:[]}, reviews:{items:[]}};
...
function openCartDrawer() {
  ...
  bindMaxCopy(overlay, `Здравствуйте! Хочу оформить заказ:\n${telegramText}\n\nИтого: ${formatPrice(total)}`);
...
function header() {
  return `<a class="skip-link" href="#main">К содержанию</a><header class="site-header" data-header><a class="brand" href="/" aria-label="THE | WESHALKA — на главную">THE <span>|</span> WESHALKA</a><nav class="desktop-nav" aria-label="Основная навигация"><a href="/catalog.html">Каталог</a><a href="/catalog.html?type=coats">Пальто</a><a href="/catalog.html?group=fur">Экошубы</a><a href="/catalog.html?type=trench">Тренчи</a><a href="/sale.html">Акции</a><a href="/collection.html">Коллекции</a><a href="/about.html">О нас</a><a href="/contacts.html">Контакты</a></nav><div class="header-actions"><button class="cart-toggle" type="button" data-cart-toggle aria-label="Корзина"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg><span class="cart-badge" data-cart-count style="display:none"></span></button><a class="header-link" href="${esc(C().telegramChannel || 'https://t.me/The_weshalka')}" target="_blank" rel="noopener noreferrer">Telegram <span>↗</span></a></div><button class="menu-toggle" type="button" aria-label="Меню" aria-expanded="false" aria-controls="mobile-menu" data-menu-toggle><span></span><span></span><span></span></button></header><div class="mobile-menu" id="mobile-menu" data-mobile-menu aria-hidden="true"><nav aria-label="Мобильная навигация"><a href="/catalog.html">Каталог <small>01</small></a><a href="/catalog.html?type=coats">Пальто <small>02</small></a><a href="/catalog.html?group=fur">Экошубы <small>03</small></a><a href="/catalog.html?type=trench">Тренчи <small>04</small></a><a href="/sale.html">Акции <small>05</small></a><a href="/collection.html">Коллекции <small>06</small></a><a href="/about.html">О нас <small>07</small></a><a href="/contacts.html">Контакты <small>08</small></a></nav><a href="${esc(C().telegramChannel || 'https://t.me/The_weshalka')}" target="_blank" rel="noopener noreferrer" class="menu-contact">Перейти в Telegram <span>↗</span></a></div>`;
}
...
function productCard(product, index = 0) {
  ...
  const currentPrice = color.price || product.price || 'Цена уточняйте';
  const oldPriceDisplay = (color.oldPrice || product.oldPrice) ? `<s class="old-price">${esc(color.oldPrice || product.oldPrice)}</s> ` : '';
  const out = !(product.colors || []).some(isAvailable);
  return `<article class="catalog-card${out ? ' is-out' : ''}" data-card data-id="${esc(product.id)}" data-category="${esc(product.category)}" data-silhouette="${esc(product.silhouette)}" data-collection="${esc(product.collection)}" data-tall="${product.tall ? 'yes' : 'no'}" data-group="${esc(productGroups(product).join(' '))}" data-season="${product.winter ? 'winter' : 'demi'}" data-price="${parseInt((color.price || product.price || '').replace(/\D/g,''),10) || ''}"><a class="catalog-card__visual" href="/product.html?id=${encodeURIComponent(product.id)}" aria-label="Открыть ${esc(product.name)}">${product.badge ? `<span class="badge">${esc(product.badge)}</span>` : ''}${media(file, alt, index < 2, '')}</a><div class="catalog-card__info"><div><small>${esc(product.collection)}${product.tall ? ' / 170+' : ''}</small><h2>${esc(product.name)}</h2><p>${esc(product.title)}</p>${swatches(product, color.id, true)}${out ? '<p class="availability availability--out">Нет в наличии</p>' : ''}</div><div class="card-price"><strong>${oldPriceDisplay}${esc(currentPrice)}</strong><a class="underlink" href="/product.html?id=${encodeURIComponent(product.id)}">Открыть <span>↗</span></a></div></div></article>`;
}
...
function groupLinks(dark = false) {
  return `<nav class="group-links ${dark ? 'group-links--dark' : ''}" aria-label="Категории каталога">${GROUPS.filter((group) => products.some((product) => productGroups(product).includes(group.id))).map((group, i) => {
...
function editorialLink() {
  const product = products.find((p) => p.id === H('editorialProduct')) || products[0];
  if (!product) return '';
  const image = H('editorialImage') || firstFile(product);
  return `<a class="editorial-image-link" href="/product.html?id=${encodeURIComponent(product.id)}" aria-label="Открыть ${esc(product.name)}">${media(image, `${product.name} — ${product.title}`)}<span class="editorial-image-link__hint">Открыть ${esc(product.name)} ↗</span></a>`;
}
function homePage() {
  const heroProduct = products[0];
  const heroColor = heroProduct.colors[0];
  const selected = (site.home?.featured || []).map((id) => products.find((p) => p.id === id)).filter(Boolean);
  ...
  shell(`<section class="hero"><div class="hero-copy"><div class="micro-row"><span>THE | WESHALKA</span><span>КОЛЛЕКЦИЯ</span></div><div class="hero-heading"><p class="eyebrow">${esc(H('heroEyebrow'))}</p><h1>${esc(H('heroTitle'))}<br><em>${esc(H('heroTitleAccent'))}</em></h1><p class="hero-lede">${esc(H('heroText'))}</p></div><div class="hero-actions"><a class="button button--dark" href="/catalog.html">Смотреть каталог <span>↗</span></a><a class="underlink" href="#editorial">О бренде <span>↓</span></a></div></div><div class="hero-visual" data-hero-showcase><div class="hero-visual__frame" data-hero-frame><div data-hero-media>${media(heroColor.files[0], `${heroColor.name}, ${heroColor.label}`, 'eager', 'hero-showcase__media')}</div></div><div class="hero-visual__top"><span data-hero-title>${esc(heroColor.name || heroProduct.name)}</span><span data-hero-collection>${esc(heroProduct.collection || 'Высокий рост 170+')}</span></div><div class="hero-visual__bottom"><span data-hero-label>${esc(heroColor.label)}</span><span><span data-hero-counter></span> <b>↓</b></span></div><div class="hero-showcase__controls" data-hero-dots aria-label="Показатели слайдшоу"></div></div></section><section class="manifesto section" id="editorial"><div class="section-mark">/ 01</div><div class="manifesto-copy reveal"><p class="eyebrow">О бренде</p><h2>Точная<br><em>форма.</em><br>Спокойный<br>характер.</h2><p>${esc(H('aboutText'))}</p><a class="underlink" href="/collection.html">Смотреть коллекции <span>↗</span></a></div></section><section class="tall-feature section--brown"><div class="tall-feature__media reveal">${media(H('tallVideo', '15271605198805.MP4'),'Видео серии для высокого роста 170+','lazy')}</div><div class="tall-feature__copy reveal"><p class="eyebrow">${esc(H('tallEyebrow'))}</p><h2>${esc(H('tallTitle'))}<br><em>${esc(H('tallTitleAccent'))}</em></h2><p>${esc(H('tallText'))}</p><a class="button button--light" href="/catalog.html?group=tall">Открыть коллекцию 170+ <span>↗</span></a></div></section><section class="section catalog-preview"><div class="section-head"><div><p class="eyebrow">Высокий рост 170+ / ${String(tallProducts.length).padStart(2,'0')}</p><h2>Модели<br><em>серии 170+</em></h2></div><a class="underlink" href="/catalog.html?group=tall">Вся серия <span>↗</span></a></div><div class="catalog-grid catalog-grid--three">${tallProducts.map((p, i) => productCard(p, i)).join('')}</div></section><section class="universal-feature section"><div class="universal-feature__copy reveal"><p class="eyebrow">Универсальная длина</p><h2>${esc(H('universalTitle'))}<br><em>${esc(H('universalTitleAccent'))}</em></h2><p>${esc(H('universalText'))}</p><a class="button button--dark" href="/catalog.html">Смотреть все модели <span>↗</span></a></div><div class="catalog-grid catalog-grid--three">${regularProducts.slice(0, 3).map((p, i) => productCard(p, i)).join('')}</div></section>${standSection}${furSection}${groupsSection}<section class="section catalog-preview"><div class="section-head"><div><p class="eyebrow">Избранные модели</p><h2>Силуэты<br><em>коллекции</em></h2></div><a class="underlink" href="/catalog.html">Весь каталог <span>↗</span></a></div><div class="catalog-grid">${selected.map((p, i) => productCard(p, i)).join('')}</div></section><section class="split-editorial"><div class="split-editorial__visual">${editorialLink()}</div><div class="split-editorial__copy"><p class="eyebrow">Классика / 03</p><h2>${esc(H('editorialTitle'))}<br><em>${esc(H('editorialTitleAccent'))}</em></h2><p>${esc(H('editorialText'))}</p><a class="underlink underlink--light" href="/collection.html">Изучить коллекции <span>↗</span></a></div></section>${(site.reviews?.items || []).length ? '' : '<!--'}<section class="reviews-preview section"><div class="section-mark">/ 04</div><div class="reviews-preview__copy"><p class="eyebrow">Отзывы</p><h2>Реальные<br><em>отзывы клиентов</em></h2><p>Покупатели делятся впечатлениями о моделях THE | WESHALKA.</p><a class="underlink" href="/reviews.html">Читать отзывы <span>↗</span></a></div></section>${(site.reviews?.items || []).length ? '' : '-->'}<section class="contact-band"><div class="contact-band__meta"><span>THE | WESHALKA / 05</span><span>CONTACT</span></div><h2>${esc(H('contactTitle'))}<br><em>${esc(H('contactTitleAccent'))}</em></h2><p>${esc(H('contactText'))}</p><div class="contact-band__buttons"><a class="contact-cta" href="${esc(C().telegramOrder || 'https://t.me/sharp_fin')}" target="_blank" rel="noopener noreferrer"><span>Telegram</span><b>↗</b></a>${maxButton('contact-cta contact-cta--max', 'MAX')}<a class="contact-cta contact-cta--wa" href="${waLink()}" target="_blank" rel="noopener noreferrer"><span>WhatsApp *</span><b>↗</b></a></div><p class="meta-disclaimer meta-disclaimer--light">* WhatsApp — продукт Meta Platforms Inc., признанной экстремистской организацией в РФ.</p></section>`, 'Верхняя одежда');
}
...
function catalogPage(collectionOnly = false) {
  ...
  const params = new URLSearchParams(location.search); if (params.get('filter') === 'tall') $('[data-filter="tall"]').value = 'yes'; if (GROUPS.some((group) => group.id === params.get('group'))) $('[data-filter="group"]').value = params.get('group'); if (params.get('type') === 'coats') { document.querySelectorAll('[data-card]').forEach((card) => { if (card.dataset.group.includes('fur')) card.dataset.typeHidden = 'yes'; }); } if (params.get('type') === 'trench') { document.querySelectorAll('[data-card]').forEach((card) => { if (!card.dataset.group.includes('trench')) card.dataset.typeHidden = 'yes'; }); } if (params.get('type') === 'fur') { $('[data-filter="group"]').value = 'fur'; }
  const applyFilters = () => { const values = Object.fromEntries([...document.querySelectorAll('[data-filter]')].map((select) => [select.dataset.filter, select.value])); let visible = 0; document.querySelectorAll('[data-card]').forEach((card) => { if (card.dataset.typeHidden === 'yes') { card.hidden = true; return; } const matches = Object.entries(values).every(([key, value]) => value === 'all' || (key === 'price' ? Number(card.dataset.price) > 0 && Number(card.dataset.price) <= Number(value) : key === 'group' ? card.dataset.group.split(' ').includes(value) : card.dataset[key] === value)); card.hidden = !matches; if (matches) visible += 1; }); $('[data-catalog-count]').textContent = `${visible} ${modelWord(visible)}`; };
...
function productPage() {
  ...
  const detail = `<section class="product-detail section" data-product-detail><div class="product-gallery"><div class="product-gallery__thumbs" data-gallery-thumbs>${galleryThumbs(activeProduct, color)}</div><div class="product-gallery__main" data-gallery-main>${media(color.files[0], productLabel(activeProduct), 'eager')}</div></div><div class="product-info"><a class="backlink" href="/catalog.html">← Вернуться в каталог</a><p class="eyebrow">${esc(activeProduct.collection)}${activeProduct.tall ? ' / 170+' : ''}</p><h1 data-product-name>${esc(color.name || activeProduct.name)}</h1><p class="product-subtitle" data-product-subtitle>${esc(color.title || activeProduct.title)}</p>${activeProduct.subtitle ? `<p class="product-alternative">${esc(activeProduct.subtitle)}</p>` : ''}${activeProduct.badge ? `<span class="badge badge--dark">${esc(activeProduct.badge)}</span>` : ''}<div class="product-price" data-product-price>${(color.oldPrice || activeProduct.oldPrice) ? `<s class="old-price old-price--dark">${esc(color.oldPrice || activeProduct.oldPrice)}</s> ` : ''}${esc(color.price || activeProduct.price || 'Цена уточняйте')}</div><div class="product-color"><div class="product-color__label"><span>Цвет</span><span data-color-label>${esc(color.label)}</span></div>${swatches(activeProduct, activeColor)}</div><div class="size-row"><label for="size">Размер</label><select id="size" data-size>${sizeOptions(color)}</select></div><p class="availability ${isAvailable(color) ? '' : 'availability--out'}" data-availability>${isAvailable(color) ? 'В наличии' : 'Нет в наличии'}</p><div class="product-actions"><button class="button button--dark" type="button" data-add-to-cart ${isAvailable(color) ? '' : 'disabled'}>Добавить в корзину</button><a class="button button--outline" data-order-link href="${telegramLink(activeProduct, color)}" target="_blank" rel="noopener noreferrer">Уточнить наличие в Telegram <span>↗</span></a>${maxButton('button button--outline', 'Уточнить в MAX')}</div><p class="product-note">🚚 ${esc(C().deliveryNote || 'Бесплатная доставка с примеркой по СПб')} &bull; 💳 Яндекс Сплит без переплаты</p><dl class="product-meta"><div><dt>Длина</dt><dd data-meta="length">${esc(color.length || activeProduct.length || 'Не указано')}</dd></div><div><dt>Состав</dt><dd data-meta="material">${esc(color.material || activeProduct.material || 'Не указан')}</dd></div><div><dt>Размеры</dt><dd data-meta="sizes">${esc(sizeText(color))}</dd></div><div><dt>Сезон</dt><dd>${esc(activeProduct.season || 'Демисезонная модель')}</dd></div></dl><div class="product-description">${String(activeProduct.description || '').split(/\n+/).filter(Boolean).map((paragraph) => `<p>${esc(paragraph)}</p>`).join('')}<ul data-details>${(color.details || activeProduct.details || activeProduct.tags || []).map((item) => `<li>${esc(item)}</li>`).join('')}</ul></div></div></section><section class="product-story section section--brown"><div><p class="eyebrow" data-story-name>${esc(color.name || activeProduct.name)}</p><h2>Форма,<br><em>которую видно.</em></h2></div><p>${esc(String(activeProduct.description || '').split(/\n+/)[0])}${activeProduct.tall ? ' Модели серии созданы с учётом длины для роста 170+.' : ''}</p></section>`;
...
function colorSizes(color) { const own = splitSizes(color.sizes); return own.length ? own : splitSizes(activeProduct.sizes); }
...
function telegramLink(product, color) { return `${C().telegramOrder || 'https://t.me/sharp_fin'}?text=${encodeURIComponent(`Здравствуйте! Подскажите, пожалуйста, наличие модели ${product.name}, цвет ${color.label}.`)}`; }
function setProductColor(id) { const color = activeProduct.colors.find((item) => item.id === id); if (!color) return; activeColor = id; const main = $('[data-gallery-main]'); const thumbs = $('[data-gallery-thumbs]'); if (main) main.innerHTML = media(color.files[0], `${activeProduct.name}, ${color.label}`, 'eager'); if (thumbs) { thumbs.innerHTML = galleryThumbs(activeProduct, color); thumbs.querySelectorAll('[data-gallery-thumb]').forEach((button) => button.addEventListener('click', () => setGalleryImage(button))); } $('[data-color-label]').textContent = color.label; const cap = document.querySelector('.color-caption'); if (cap) cap.textContent = color.label; $('[data-product-name]').textContent = color.name || activeProduct.name; $('[data-product-subtitle]').textContent = color.title || activeProduct.title; $('[data-story-name]').textContent = color.name || activeProduct.name; $('[data-product-price]').innerHTML = `${(color.oldPrice || activeProduct.oldPrice) ? `<s class="old-price old-price--dark">${esc(color.oldPrice || activeProduct.oldPrice)}</s> ` : ''}${esc(color.price || activeProduct.price || 'Цена уточняйте')}`; $('[data-meta="length"]').textContent = color.length || activeProduct.length || 'Не указано'; $('[data-meta="material"]').textContent = color.material || activeProduct.material || 'Не указан'; $('[data-meta="sizes"]').textContent = sizeText(color); const sizeSelect = $('[data-size]'); if (sizeSelect) sizeSelect.innerHTML = sizeOptions(color); $('[data-details]').innerHTML = (color.details || activeProduct.details || activeProduct.tags || []).map((item) => `<li>${esc(item)}</li>`).join(''); $('[data-order-link]').href = telegramLink(activeProduct, color); const av = $('[data-availability]'); if (av) { av.textContent = isAvailable(color) ? 'В наличии' : 'Нет в наличии'; av.classList.toggle('availability--out', !isAvailable(color)); } const addBtn = $('[data-add-to-cart]'); if (addBtn) addBtn.disabled = !isAvailable(color); document.querySelectorAll('[data-color]').forEach((button) => { const active = button.dataset.color === id; button.classList.toggle('is-active', active); button.setAttribute('aria-pressed', String(active)); }); }
...
function simplePage(page) {
  ...
  else if (page === 'sale') salePage();
  else if (page === 'reviews') reviewsPage();
}
...
<a class="contact-cta" href="${esc(C().vk || 'https://vk.ru/club161262776')}" target="_blank" rel="noopener noreferrer"><span>ВКонтакте</span><b>↗</b></a>
<a class="contact-cta contact-cta--secondary" href="${esc(C().telegramChannel || 'https://t.me/The_weshalka')}" target="_blank" rel="noopener noreferrer"><span>Telegram</span><b>↗</b></a>
</div>
...
<video src="${esc(asset(C().showroomVideo || 'showroom-directions.mp4'))}" controls playsinline preload="metadata" style="width:100%;max-width:480px;border-radius:12px;margin-top:12px" aria-label="Как найти шоурум THE | WESHALKA"></video>
</div>
</div>
</div>
...
function salePage() {
  const items = site.sale?.items || [];
  const body = items.length
    ? `<div class="sale-grid">${items.map((item) => `<article class="sale-card">${item.image ? `<img src="${esc(asset(item.image))}" alt="${esc(item.title || '')}" loading="lazy">` : ''}<h2>${esc(item.title || '')}</h2>${item.text ? `<p>${esc(item.text)}</p>` : ''}${item.link ? `<a class="underlink" href="${esc(item.link)}">Подробнее <span>↗</span></a>` : ''}</article>`).join('')}</div>`
    : `<p style="font-size:18px;color:var(--muted);max-width:500px;margin:0 auto">${esc(site.sale?.emptyText || 'Следите за обновлениями — скоро здесь появятся актуальные акции и специальные предложения.')}</p>`;
  shell(`<section class="page-hero"><p class="eyebrow">THE | WESHALKA</p><h1>Акции</h1><p>${esc(site.sale?.intro || 'Специальные предложения и скидки на верхнюю одежду.')}</p></section>
<section class="section" style="text-align:center;padding:80px 7vw">
${body}
<a class="button button--dark" href="/catalog.html" style="margin-top:2rem">Перейти в каталог</a>
</section>`, 'Акции');
}
function reviewsPage() {
  const items = site.reviews?.items || [];
  const list = items.length
    ? `<div class="reviews-grid">${items.map((review) => `<article class="review-card"><div class="review-stars">${'★'.repeat(Math.max(1, Math.min(5, Number(review.rating) || 5)))}</div><p class="review-text">${esc(review.text || '')}</p><div class="review-author"><strong>${esc(review.name || '')}</strong>${review.city ? `<span>${esc(review.city)}</span>` : ''}</div></article>`).join('')}</div>`
    : '<p class="review-empty">Отзывы скоро появятся. Поделиться впечатлениями о покупке можно в Telegram.</p>';
  shell(`<section class="page-hero"><p class="eyebrow">THE | WESHALKA / 04</p><h1>Отзывы<br><em>клиентов</em></h1><p>${esc(site.reviews?.intro || '')}</p></section><section class="reviews-page section">${list}<a class="button button--dark" href="${esc(C().telegramOrder || 'https://t.me/sharp_fin')}" target="_blank" rel="noopener noreferrer" style="margin-top:2rem">Написать в Telegram <span>↗</span></a></section>`, 'Отзывы');
}
function showroomDetails() {
  const c = C();
  return `<div class="showroom-detail"><strong>📍 Адрес</strong><p>${esc(c.address || '')}<br>${esc(c.metro || '')}</p></div>
...
async function init() { try { loadCart(); const [catalogResponse, siteResponse] = await Promise.all([fetch('/data/catalog.json', {cache:'no-cache'}), fetch('/data/site.json', {cache:'no-cache'})]); const catalog = await catalogResponse.json(); try { site = Object.assign(site, await siteResponse.json()); } catch {} products = (catalog.products || []).filter((product) => !product.hidden && (product.colors || []).length).map((product) => ({...product, groups: product.groups || [], tall: (product.groups || []).includes('tall'), colors: product.colors.filter((color) => (color.files || []).length)})).filter((product) => product.colors.length); collectPosters(); bindVideoFallback(); const page = document.body.dataset.page; if (page === 'home') homePage(); else if (page === 'catalog') catalogPage(false); else if (page === 'collection') catalogPage(true); else if (page === 'product') productPage(); else simplePage(page); } catch (error) { console.error('THE | WESHALKA data error', error); document.body.innerHTML = '<main class="error-page"><h1>Каталог временно недоступен</h1><a href="/">Вернуться на главную</a></main>'; } }
